...
package=`php "$directory/parsejava.php" --package "$PWD/$CR_FILENAME" 2>/dev/null`
...
javac "$CR_FILENAME" -encoding $CR_ENCODING "${@:1}"
status=$?
...
out=`php "$directory/parsejava.php" "$PWD/$CR_FILENAME" 2>/dev/null`
status=$?
if [ $status -ne 0 ]; then
